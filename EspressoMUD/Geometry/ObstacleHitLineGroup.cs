...
namespace EspressoMUD
{
    public class ObstacleHitLineGroup
    {
        ...
        private List<ObstacleHitLineGroup> startPointOverlaps = new List<ObstacleHitLineGroup>();
        ...
        public void SetupPoints(List<Point> points, VolumeTree<ObstacleHitLine> collectionToPopulate, Vector surfaceNormal)
        {
            ...
            for (int i = 0; i < points.Count - 2; i++)
            {
                ...
                //for (int j = startIndex; j < stop; j++)
                //{
                //    Point secondStart = secondEnd;
                //    secondEnd = EndPointOf(j);
                ...
                //            Point previousStart = StartPointOf(j - 1);
                ...
                //        StructListStruct<int>.Add(ref linesToSkip, j);
                //        nextI = true;
                //        if (intersect.Equals(secondStart))
                //        {
                ...
                //    if (wasI >= 2 != isI >= 2)
                //    {
                //        newOverlap = new HitLineSetupOverlapData() { inside = (isI >= 2), intersect = intersect };
                //        StructListStruct<HitLineSetupOverlapData>.Add(ref setupOverlaps[i + (nextI ? 1 : 0)], newOverlap); //TODO: This needs a conditional i+1
                //    }
                //    if (wasJ >= 2 != isJ >= 2)
                //    {
                //        newOverlap = new HitLineSetupOverlapData() { inside = (isJ >= 2), intersect = intersect };
